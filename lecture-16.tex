...
\input{header}
\title{Lecture-16: Limit Theorems and the Laws of Large Numbers}
\author{}

\begin{document}
\maketitle

\subsection{Weak law of large numbers, strong law of large numbers and central limit theorem for a sequence of iid random variables}
\subsection{Proof of weak law of large numbers for the case of uncorrelated sequence satisfying a bounded variance assumption}
\subsection{Definition of a random process; $n^{\text{th}}$-order joint distribution of a random process, $n^{\text{th}}$-order joint pmf/pdf}
\subsection{Mean function and correlation function of a random process}
\subsection{Strict-sense stationarity and wide-sense stationarity of random processes}
\subsection{Correlation function of a wide-sense stationary process is a function of only one argument}
\subsection{Jointly stationary random processes}
\end{document}

...
\input{header}
\title{Lecture-13: Random Processes}
\author{}
...
%\begin{defn} 
A  collection of random variables $\{X_t \in \sX: t \in T\}$ each defined on the same probability space $(\Omega, \sF, P)$ is called a \textbf{random process} for an arbitrary index set $T$ and arbitrary state space $\sX$. 
%\end{defn}
